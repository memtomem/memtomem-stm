...
async def app_lifespan(server: MCPServer) -> AsyncIterator[STMContext]:
    ...
    # them, and ``None`` (the probe failed) makes it sweep nothing rather than
    # treat every pre-existing child as a leak. A host that spawns its own
    # subprocesses *after* mounting us cannot be told apart from a leak by this
...
async def stm_progressive_stats(
    ...
    lines = [
        ...
        f"Recorded initial payload chars: {stats['initial_payload_chars']}",
        f"Recorded follow-up payload chars: {stats['follow_up_payload_chars']}",
        f"Unclassified legacy read events: {stats['unclassified_reads']}",
        "Read volumes exclude footers/surfacing and count repeated reads again.",
        "Telemetry is best-effort; absent events do not establish zero delivery.",
