...
| `sensitive_metadata` | credential-pattern match in the tool's metadata: the composed `name`, the advertised description and the raw one behind it, the raw input schema, the forwarded `outputSchema` / `_meta`, and `annotations` in the tagged form the client receives (the server name is prepended to a non-empty `title`). The raw artifacts are scanned even where truncation would have cut the match, since truncation only removes text. A field that cannot be serialized at all counts as a match — a value the gate cannot read is one it cannot clear | rejects under `strict`; demotes under `review` |
