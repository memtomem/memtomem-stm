...
The visible consequence is ordering. Fault counters, score-scale diagnostics and the hourly retention sweeps are queued when their branch runs, so they are no longer guaranteed to land before the response — an idle worker may still beat it, a busy one will not. The worker is a single FIFO thread, which keeps rows in the order the engine asked for them but also means an awaited write — the event row a call needs before it can advertise a feedback ID — waits for whatever is queued ahead of it. That wait is capped at `timeout_seconds`; past the cap the call delivers its memories without a feedback prompt, and the write stays queued.
