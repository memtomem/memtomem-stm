...
Providers: `openai`, `anthropic`, `ollama`. `base_url` (default `""`) overrides the provider endpoint — required to point `ollama` at a non-default host, and how OpenAI/Anthropic get aimed at a compatible gateway. `llm_timeout_seconds` (default 60.0) bounds the per-call LLM wait. On timeout, privacy-pattern hit, circuit-breaker open, or any other API failure, compression falls back to `TruncateCompressor` and records the strategy as `llm_summary→{timeout,privacy,circuit_breaker,llm_error}_fallback` in `proxy_metrics` for observability. A successful call whose summary still exceeds `max_chars` is clamped by the same truncation and recorded as `llm_summary→llm_overlength_fallback` — the model overshot the length instruction; the endpoint is fine, so the circuit breaker is unaffected. A call whose summary comes back empty or whitespace-only falls back to the truncated **original** text — same class as overshoot (the endpoint answered; the model produced nothing usable), so the breaker is likewise unaffected. The compressor reports `llm_empty`, but the **recorded** strategy is whatever the pipeline settles on: the retention ratio guard runs afterwards, so if the truncated fallback is still below the floor the ladder replaces it and the row reads (for example) `llm_summary→progressive_fallback` instead. The row shows the compressor's reason only when the fallback clears the floor, or when no floor is set. Without that label such a call returned the empty string through the success path, destroying the payload while booking a successful compression.
