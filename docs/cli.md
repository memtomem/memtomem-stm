...
filtered out by name and by the identity signature described under
[`prune`](#prune) — `(transport, command, args, env)` /
`(transport, url, headers)` — before the selection UI. `--validate` and `--timeout` work on the
selected subset.
...
"Dual-registered" requires both the name **and** the identity to match: the source-client entry must share the STM upstream's identity signature: transport plus command and args for stdio, transport plus URL for HTTP, and in both cases the connection settings — `env` for stdio, `headers` for HTTP. Those settings are part of it because the same command line against a different database, tenant or token is a different server (#983). They are compared in the normalized form an import produces: values are compared as strings (a host `8080` and a config `"8080"` are one setting), header *names* fold case (RFC 9110) while their values do not, environment names are compared case-sensitively, and an absent, `null` or empty block is the same as no block. Env names are case-sensitive by choice, not by platform: Windows resolves them case-insensitively, but a config file is portable and which server it names must not depend on the machine reading it, so `DB` and `db` are two settings everywhere — on Windows that costs you a prune that declines and an import that adopts a second entry, never a deletion. Two consequences worth knowing: a difference confined to an env key import strips (`LD_PRELOAD`, `PYTHONPATH` and the rest of the [dangerous set](#dangerous-env-keys)) is not a difference here, because no import would carry it either way and the verbatim host entry is still captured in the entry's `origin` at import time and appended to the prune backup log before any deletion; and an `env` or `headers` block on a *registered* entry that is present but is not a mapping is a claim that cannot be read, so `mms prune` skips that entry and an import declines a same-command candidate against it, exactly as a broken `origin` does. Only the block the transport reads counts — a stdio `headers` and an HTTP `env` are inert here as they are in the signature. The same shape in a *host* config is dropped at import normalization, as it always has been — so identity cannot see it, and that entry is held apart the same way rather than compared as one holding no block: `mms prune` skips it, an import declines it with `ambiguous_identity` (a candidate whose *name* is already registered is still reported as `already_registered`, which is checked first), `mms init` leaves it out of the servers it offers, and a same-name entry in a second client is recorded as a conflict rather than a duplicate to prune (#984). Discovery says so when it reads one, naming the field and the source but never the value. A stdio `cwd` is compared on top of that, but only where both sides state one. Host configs have no `cwd` field, so the checkout is inferred per side: a checkout-scoped source (`.mcp.json`, a Claude Code project slot, `.cursor/mcp.json`) contributes the checkout its config belongs to, and a registered upstream contributes the one recorded in its `origin`. A checkout-less source (Claude Code user scope, Claude Desktop, `~/.cursor/mcp.json`) contributes nothing and matches any `cwd` — those servers do run from wherever the client starts, and so does an entry with no `origin` block (a manual `mms add`, or an import predating provenance capture). So identical relative commands in two checkouts stay distinct: `mms prune` run from the second one leaves that checkout's unrelated entry alone. An entry whose `origin` *does* name a checkout that this build cannot resolve — a malformed block, an unrecognized source kind, or a path that is missing, relative, or not anchored where that kind anchors it — is skipped rather than pruned: a claim that cannot be read is not a claim to run anywhere. On the import side the same entry declines a same-command candidate with `ambiguous_identity` rather than importing a second copy of a server it may already proxy. Only stdio identity has a checkout; an HTTP entry is its URL wherever it was registered from, so a broken `origin` never makes one incomparable (a broken `headers` block still does). If you've edited either side to point at a different server that happens to share a name, `mms prune` skips it rather than clobbering the unrelated entry.

The same comparison decides what a *second* source client claiming a name means — the same rule, so the same limits: identity is the signature above plus `cwd`. A same-name entry whose identity matches the first source's is the same server registered twice, and every source `mms` can write is pruned so the dual path collapses. A same-name entry whose identity *differs* is a third state: it is named in the preview, in the confirm prompt and in `conflicts` under `--json`, and then left exactly as it is — not planned, not written, not backed up, and not recorded as a duplicate in the entry's `origin`. No manual edit is suggested for one either, unlike a Cursor row: there is nothing to remove, because that entry is not this server's registration. The note names the server and the source holding the other one and stops there: that row describes a server you did not ask to act on, and a host command line carries credentials in its arguments and URLs. Two caveats worth knowing: `mms init` filters out repo-local candidates before this list is built, so a `.mcp.json` entry that wins a contested name takes the conflict off `init`'s list with it (`mms add --from-clients --allow-project-configs` shows both); and when the *first* source's entry is the one that diverges from your STM upstream, the whole name is skipped, so a matching entry in a later source is not pruned either.

Your upstream definitions in `stm_proxy.json` are never modified — a prune changes source-client files, not what STM proxies. It does write one thing back: for an entry that carries an `origin` block, each source it successfully removed is stamped `pruned` there, which is what lets [`mms list`](#list) mark the entry and [`mms eject`](#eject) know what is left to restore. Failures are surfaced via non-zero exit and the exact manual fallback for each failed entry — a `claude mcp remove` command for the shell-out scopes, the config edit to make for Claude Desktop — so scripting callers can retry.
...
With `--json`, stdout carries a single result document: `{"action": "prune", "ok": ..., "dry_run": ..., "config_path": ..., "planned": [{"name", "source"}], "manual": [{"name", "source", "hint"}], "conflicts": [{"name", "source"}], "pruned": [...], "failed": [{"name", "source", "error", "hint"}]}` — `planned` contains only sources `mms` will attempt to write; `manual` contains unmanaged sources (currently Cursor) with the edit to make; `conflicts` contains sources holding a different server under the same name, which carry no `hint` because there is no edit to recommend. Each actual `failed` row's `hint` is the exact manual removal command. A manual-only run exits 0 with `ok: true` because no planned action failed. `--json` never prompts: without `--yes` (or `--dry-run`) it refuses with exit 2 and `{"error": "confirmation_required", ...}`. Partial failures keep `ok: false` + exit 1, with the human diagnostics still on stderr.
...
- **No-clobber.** A same-name entry already at the target is compared by the same identity rule [`prune`](#prune) uses — command/URL, args and `env`/`headers`: identical → idempotent skip; different or not comparable → that entry fails with a hint unless `--force` (which replaces it). Identity is the gate on *overwriting* someone else's entry; the release of the STM entry is gated separately and more strictly, by the deep-equality check below.
...
| `description budget: <name>` | the advertisement's own budget rule (`min(server, global)` cap, `[proxied] ` prefix, convention suffix) over the tool lengths the staged probe collected | WARN: the cap would cut discovered descriptions, or the resolved compression strategy's convention suffix cannot fit it. Names the smallest cap that would carry every description and its hint whole, and every config level below that number, since the budget is `min(server, global)` and raising one level alone stops at the next. Counts cover discovered tools minus config-hidden ones; exposure filtering can withhold more, and the reported excess is the amount over the budget, which a boundary retreat can exceed. Never FAIL — the cap is a valid choice; going unnoticed is the fault. Computed from the config file, so it describes the next start, not the running proxy, and a host-side cut is a separate limit it does not measure |
