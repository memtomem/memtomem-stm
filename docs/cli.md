...
`prune` lists them as manual-only, import-time `--prune` reports the refused
removal with the manual edit, and `eject` does not accept a `cursor-*` target.
