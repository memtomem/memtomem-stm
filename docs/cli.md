...
| `surfacing outcomes` | daemon `latency.surface` counters | WARN when any hook surfacing request since daemon start timed out, failed mid-search, or never issued one, with that breakdown; cold starts and healthy skips (allowlist, gate, cache) are not failures, and raw/synthetic `retrieval` traffic is not counted here — see `ltm measurement`. Independent of whether there is enough data to size a timeout |
