...
"Dual-registered" requires both the name **and** the identity to match: the source-client entry must share the STM upstream's `(transport, command, args)` or `(transport, url)` signature. A stdio `cwd` is compared on top of that, but only when **both** entries record one — host configs never do, so a `cwd` on the STM side alone does not make the pair strangers, while two recorded and different `cwd`s keep identical relative commands in separate checkouts distinct. If you've edited either side to point at a different server that happens to share a name, `mms prune` skips it rather than clobbering the unrelated entry.
