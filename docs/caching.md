...
- Cache key = SHA-256 over `server`, `tool`, `args` (argument order independent), the call's `_context_query`, and a fingerprint of the settings that shape the stored body — the resolved per-tool compression settings plus the `min_result_retention` and `max_upstream_chars` globals. The stored body is the *shaped + compressed* response — compression is query-aware (BM25 relevance budgets) and config-dependent — so the same tool+args under a different query context, or after a shaping/compression-setting change (hot reload included), gets its own entry instead of being served a body produced for someone else's query or under old settings. The flip side: callers that pass a different `_context_query` on every call re-fetch instead of hitting the cache.
