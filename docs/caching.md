...
- **Envelope-aware successful text responses** — cache schema v4 stores the compressed text plus JSON-safe `structuredContent` and result-level `_meta`, then reconstructs the same `CallToolResult` envelope on a hit. Privacy scanning covers both text and envelope JSON. Errors and non-text/mixed content remain uncached; malformed, non-JSON-safe, or unmarked out-of-band rows are never served.
