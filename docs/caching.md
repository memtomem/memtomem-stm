...
- Cache key = SHA-256 over length-framed `server`, `tool`, `args` (argument order independent), the call's `_context_query`, and a fingerprint of the settings that shape or admit the stored body — the resolved per-tool compression settings plus the `min_result_retention`, `max_upstream_chars`, `max_upstream_bytes`, and `relevance_scorer` globals. Length framing prevents component-boundary collisions. The stored body is the *shaped + compressed* response — compression is query-aware (BM25 relevance budgets) and config-dependent — so the same tool+args under a different query context, or after a shaping/compression/admission-setting change (hot reload included), gets its own entry instead of being served a body produced for someone else's query or under old settings. The flip side: callers that pass a different `_context_query` on every call re-fetch instead of hitting the cache.
...
- **Envelope-aware successful text responses** — cache schema v5 stores the compressed text plus JSON-safe `structuredContent` and result-level `_meta`, then reconstructs the same `CallToolResult` envelope on a hit. Privacy scanning covers both text and envelope JSON. Errors and non-text/mixed content remain uncached; malformed, non-JSON-safe, or unmarked out-of-band rows are never served.
...
- Clear cache via MCP tool: `stm_admin(action="proxy_cache_clear", params={"server": "gh", "tool": "search_code"})` clears matching response-cache entries; an unfiltered `stm_admin(action="proxy_cache_clear")` flushes the entire response cache **and** the in-memory surfacing result cache
