...
- **Tuning ratio advice ignores rows of unknown accounting** (#1039). The
  ...
  path does not read.
- **An explicitly configured server `max_result_chars` of 8000 now applies**
  (#1048, #1040). That one value was read as if the field had been omitted, so the
  server inherited the model-aware global instead — commonly the 16,000-character
  `default_max_result_chars`. Adjacent values were unaffected: 7999 resolved to
  7999 and 8001 to 8001. Configurations written by earlier versions of `mms add`
  and `mms init` carry that literal, so those servers drop from 16,000 to 8,000
  characters on upgrade. Nothing is rewritten automatically: delete the server's
  `max_result_chars` to restore inheritance, or set the value you want. An entry
  that also carries a token budget is unaffected, because a token budget outranks
  a character one — `mms init --lang ko` entries stay at 3,700 characters.
- **`mms add --max-chars` no longer has a default** (#1048). It used to write
  `8000` into every generated entry whether or not you passed the flag, and
  `--help` advertised `[default: 8000]`. The field is now written only when the
  flag is passed, and the entries generated by `mms init` (imported, demo, and
...
- **Explicit progressive and progressive fallback share one accounting basis**
  ...
  above.
- **A server budget equal to the schema default is a stated budget** (#1048) —
  `effective_max_result_chars()` compared the configured value against
  `UpstreamServerConfig`'s field default rather than asking whether the field was
  supplied, so the one value equal to that default could not be stated. It now
  reads `model_fields_set`, the same signal `compression` already uses to separate
  a stated strategy from an inherited one, and the distinction survives file
  loading, the `MEMTOMEM_STM_PROXY__UPSTREAM_SERVERS__*` environment overlay, and
  the CLI's own load/save path. Tool overrides, token-budget precedence, retention
  floors, and model-aware global scaling are unchanged. Every config generator
