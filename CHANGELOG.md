...
- **`SurfacingConfig.injection_mode` default flipped from `prepend` to `append`** (#348) — the previous default silently disabled surfacing on the progressive-delivery path. `ProxyManager._apply_surfacing_on_progressive` short-circuits when `injection_mode == "prepend"` because prepending memories would shift character offsets and break the `PROGRESSIVE_FOOTER_TOKEN` concat invariant `stm_proxy_read_more` depends on. With `prepend` as the default, every progressive call (and ratio-guard progressive fallback) under a fresh `mms init` config emitted exactly one WARNING and then returned the response unchanged — exactly the largest responses for which surfacing is most useful. `append` preserves the concat invariant and lets the progressive path surface by default; `prepend` remains valid and is still useful for non-progressive responses. **Behavior change**: operators who relied on the implicit `prepend` default now get memories appended to the response instead of prepended; explicit `injection_mode: prepend` settings are unaffected. The progressive short-circuit also now records a `progressive_mode_conflict` skip in `stm_surfacing_stats` so the bypass is visible after process restart instead of relying on the one-shot WARNING. New `SkipReason` value is intentionally mode-agnostic so a future per-mode constraint can reuse it.

- **STM observability/admin MCP tools are now opt-in by default** (#228 phase 2) — `MEMTOMEM_STM_ADVERTISE_OBSERVABILITY_TOOLS` now defaults to `false`, so the MCP surface advertises only the four model-facing STM tools (`stm_proxy_read_more`, `stm_proxy_select_chunks`, `stm_surfacing_feedback`, `stm_compression_feedback`) plus proxied upstream tools unless the operator explicitly sets `MEMTOMEM_STM_ADVERTISE_OBSERVABILITY_TOOLS=true`. The eight observability/admin tools are not registered with the MCP server by default and can be re-enabled over MCP with the env var. This preserves the phase-1 ordering fix while reducing picker noise and schema load for clients that eager-load every advertised tool.

### Fixed

- **`stm_surfacing_stats` readiness label surfaces both per-tool and global-pool gaps** (#361, #351 part 4) — the previous label rendered only `need N more for auto-tune` derived from the global-pool shortfall, even though the underlying mechanic (`feedback.py::maybe_adjust` cold-start fallback) considers per-tool and global counts independently. Operators reasonably read the single figure as their own tool's shortfall, so a row with `feedback_count=2` and label `need 18 more` looked like it needed 16 more own-feedback events when the global pool would actually tune it on the very next surfacing. The label now reads `need 13 more (global pool) or 18 more for this tool` whenever the two gaps diverge, naming each side explicitly; when a single tool owns the entire feedback pool the gaps coincide and the legacy single-number form (`need N more for auto-tune`) is preserved to avoid redundant text. Two regression tests pin both branches: the divergent case (`test_need_more_uses_global_gap_when_pool_also_below_threshold`) and the equal-gap case (`test_need_more_renders_single_number_when_one_tool_owns_pool`). The other three #351 gaps (verdict line / skip-reason healthy-vs-fault split / helpfulness target band) track separately as #362-#364.

- **Surfaced-memories block keeps the rating loop alive under truncation and enumerates valid ratings** (#350, parts 3+4) — two related UX bugs in `SurfacingFormatter.inject` that silently broke the feedback signal `AutoTuner` learns from. (1) The `surfacing_id` line was appended at the bottom of `lines` *before* the `effective_max_injection_chars` cut, so the largest, most expensive surfacings — exactly the ones operators most want feedback on — could lose the ID entirely, leaving the agent with memories but no way to call `stm_surfacing_feedback`. The ID and the rating spec now live directly after `section_header`, above the bullet list, so they survive any truncation that leaves the header itself intact. (2) The previous italic hint (`_Surfacing ID: ... — call \`stm_surfacing_feedback\` to rate_`) did not enumerate valid rating values; agents routinely guessed `"good"` / `"useful"` and `FeedbackTracker.record_feedback` rejected them, so the tuner never saw the signal. The rating spec is now formatted as an explicit tool invocation (`> Rate (one of "helpful" | "not_relevant" | "already_known"): \`stm_surfacing_feedback(surfacing_id="abc", rating="helpful")\``) — the alternatives are listed in prose *before* the call so the callable itself remains copy-pasteable; pipe-alternation inside `rating=` would parse as `BinOp(BitOr)` and be rejected by the validator, defeating the very signal this PR preserves. The enumeration is sourced from `feedback.VALID_RATINGS` (renamed from private `_VALID_RATINGS`) so the agent-visible spec and the server-side validator cannot drift. **Behavior change**: the rendered surfaced-memories block reorders the ID line from the bottom to just below the header, and the trailing `_Surfacing ID: ...` italic line is gone — any tooling that scraped the bottom line for the ID must read the new top-of-block format (regex `surfacing_id[:=]\s*"?([a-f0-9]{16})` matches both the italic line and the rating-spec callable). Parts 1 (source rendering) and 2 (score abstraction) of #350 are deferred — part 2 in particular needs measured "agent ignores sub-X scores" signal before changing the score display contract.

- **Surfacing logs a one-shot WARNING when the LTM MCP command is unreachable** (#349, part a) — when `surfacing.enabled=True` (the default) and the configured `surfacing.ltm_mcp_command` (default `memtomem-server`) is not on PATH or fails to start, `McpClientSearchAdapter` flips a sticky `_start_attempted=True` and returns `outcome="no_session"` for every subsequent call. The engine maps that to `record_skip(tool, "ltm_unavailable")` so the bypass is *counted*, but operators previously only had the counter in `stm_surfacing_stats` to learn that surfacing was silently disabled for the process lifetime. `SurfacingEngine` now logs a single `WARNING` on the first `no_session`/`transport_error` outcome naming the configured command and pointing at `mms health` for active diagnosis. The counter still increments on every subsequent skip and the WARNING does not repeat — symmetric to the prepend-on-progressive WARNING-once pattern (#348). Part b (`mms health` LTM probe) tracks separately under #349; the init-time prompt proposal is deferred wait-for-signal.

- **Surfacing AutoTuner now treats `already_known` as negative feedback** (#347) — `AutoTuner.maybe_adjust` previously asked `FeedbackStore.get_tool_not_relevant_ratio`, which only counted `rating == 'not_relevant'` in the numerator while keeping every rating in the denominator. A tool that received only `already_known` ratings (the agent telling STM "stop surfacing this, I know it already") therefore reported ratio 0.0 and crossed the `< 0.2` branch — *lowering* `min_score` and surfacing **more** of the same memories on the next call, the opposite of the agent's signal. The ratio now counts `('not_relevant', 'already_known')` as the negative numerator (Option A from the issue); the AutoTuner log line and `docs/surfacing.md` table read "negative ratio" instead of "not_relevant ratio". `per_tool_breakdown` rows in `stm_surfacing_stats` gain a `negative_count` field; `not_relevant_count` is preserved for backward compatibility, and the `server.py` formatter falls back to it when `negative_count` is absent. The legacy `FeedbackStore.get_tool_not_relevant_ratio` method is also preserved. **Behavior change**: operators with persisted per-tool adjustments learned via the broken ratio may want to delete the AutoTuner adjustment rows (or accept that the tuner will re-converge from the corrected ratio over the next `auto_tune_min_samples` feedback events per tool). Regression coverage: `test_already_known_raises_threshold` pins the direction flip; `test_negative_ratio_counts_already_known` pins the new numerator.
